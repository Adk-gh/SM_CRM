          ...
          <div className="flex-grow overflow-y-auto p-6 sm:p-10 bg-[#F4F4F4] dark:bg-[#2C3E50]">
            {uiState.submitted ? (
              <div className="flex flex-col items-center justify-center text-center max-w-xl mx-auto py-10">
                <div className="mb-6 flex justify-center w-full"><CheckCircle className="w-20 h-20 text-[#48BB78] dark:text-[#68D391]" /></div>
                <h2 className="text-[1.75rem] font-bold mb-3 text-[#395A7F] dark:text-[#E9ECEE]">Submission Confirmed!</h2>
                <p className="text-lg mb-8 leading-relaxed max-w-[80%] text-[#7A7A7A] dark:text-[#B0B0B0]">A confirmation email has been sent to <strong>{formData.userEmail}</strong>.</p>
                <div className="bg-[#A3CAE9] text-[#395A7F] px-6 py-4 rounded-xl mb-8 font-bold text-lg border border-[#395A7F] shadow-sm inline-block break-all max-w-full">Ticket ID: {uiState.ticketId}</div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-0 bg-[#F4F4F4] dark:bg-[#2C3E50]">
                ...
                <div className="mb-10 pb-8 border-b border-[#D1D5DB] dark:border-[#4A5568]">
                  <h3 className={sectionTitleClasses}><User className="w-6 h-6 text-[#395A7F] dark:text-[#63B3ED]" /> Contact Information</h3>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="flex flex-col">
                      <label htmlFor="userName" className={labelClasses}>Full Name *</label>
                      <input type="text" id="userName" name="userName" value={formData.userName} onChange={handleInputChange} placeholder="Enter your full name" required className={inputClasses} />
                    </div>

                    <div className="flex flex-col">
                      <label htmlFor="userEmail" className={labelClasses}>Email Address *</label>
                      <input type="email" id="userEmail" name="userEmail" value={formData.userEmail} onChange={handleInputChange} placeholder="Enter your email" required className={inputClasses} />
                    </div>
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="relative">
                        <select id="smBranch" name="smBranch" value={formData.smBranch} onChange={handleInputChange} required className={`${inputClasses} appearance-none cursor-pointer`}>
                          {SM_BRANCHES.map(branch => <option key={branch.value} value={branch.value}>{branch.label}</option>)}
                        </select>
                        <div className="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none text-gray-500"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg></div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-0">
                  <h3 className={sectionTitleClasses}><AlertCircle className="w-6 h-6 text-[#395A7F] dark:text-[#63B3ED]" /> Issue Details</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="col-span-1 md:col-span-2 flex flex-col">
                      <label htmlFor="issueCategory" className={labelClasses}>Category *</label>
                      <div className="relative">
                        <select id="issueCategory" name="issueCategory" value={formData.issueCategory} onChange={handleInputChange} required className={`${inputClasses} appearance-none cursor-pointer`}>
                    ...
                    <div className="col-span-1 md:col-span-2 flex flex-col">
                      ...
                      <input type="text" id="issueTitle" name="issueTitle" value={formData.issueTitle} onChange={handleInputChange} placeholder="Brief summary of the issue" required className={inputClasses} />
                    </div>

                    <div className="col-span-1 md:col-span-2 flex flex-col">
                      <label htmlFor="issueDescription" className={labelClasses}>Description *</label>
                      <textarea id="issueDescription" name="issueDescription" value={formData.issueDescription} onChange={handleInputChange} placeholder="Please describe the issue in detail..." required rows="6" className={`${inputClasses} min-h-[120px] resize-y leading-relaxed`} />
                    </div>
                  </div>
                  ...
                  <div className="mt-5 col-span-1 md:col-span-2 flex flex-col">
                    ...
                    <div 
                      className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-300 mb-4 bg-white dark:bg-[#1E2A38] ${uiState.dragActive ? 'border-[#395A7F] dark:border-[#63B3ED] bg-[#A3CAE9] dark:bg-[#4299E1] -translate-y-[1px] shadow-md' : 'border-[#D1D5DB] dark:border-[#4A5568] hover:border-[#395A7F] dark:hover:border-[#63B3ED] hover:bg-[#A3CAE9] dark:hover:bg-[#4299E1]/20 hover:-translate-y-[1px] hover:shadow-sm'}`}
                    ...
                    >
                      ...
                      <div className="mb-3 text-[#395A7F] dark:text-[#63B3ED] flex justify-center"><Paperclip className="w-12 h-12" /></div>
                      <p className="text-base text-[#57789B] dark:text-[#A3CAE9] font-medium mb-2"><span className="text-[#395A7F] dark:text-[#63B3ED] font-semibold">Click to upload</span> or drag files here</p>
                      ...
                      <div className="flex flex-col gap-2">
                        {formData.files.map((file, idx) => (
                          <div key={idx} className="flex justify-between items-center p-4 bg-white dark:bg-[#1E2A38] border border-[#D1D5DB] dark:border-[#4A5568] rounded-xl transition-all duration-200 shadow-sm hover:border-[#6E9FC1] dark:hover:border-[#68D391] hover:-translate-y-[1px] hover:shadow-md">
                            <div className="flex items-center gap-3 font-medium"><FileText className="w-5 h-5 text-[#7A7A7A] dark:text-[#B0B0B0]" /><span className="text-sm sm:text-base text-[#57789B] dark:text-[#A3CAE9]">{file.name}</span></div>
                            <button type="button" onClick={() => removeFile(idx)} className="bg-transparent border-none text-[#F56565] cursor-pointer flex items-center p-1 rounded-md transition-all duration-200 hover:bg-[#F56565]/10 hover:scale-110"><X className="w-4 h-4" /></button>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                <button 
                  ...
                  className={`mt-6 w-full flex items-center justify-center gap-2.5 rounded-xl border-none bg-gradient-to-br from-[#395A7F] to-[#2F4D69] dark:from-[#63B3ED] dark:to-[#4299E1] text-white text-base font-semibold px-8 py-4 tracking-wide cursor-pointer transition-all duration-300 shadow-[0_4px_12px_rgba(57,90,127,0.2)] ${(!captchaToken || uiState.loading) ? 'opacity-60 cursor-not-allowed grayscale' : 'hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(57,90,127,0.3)] hover:bg-gradient-to-br hover:from-[#6E9FC1] hover:to-[#395A7F] dark:hover:from-[#68D391] dark:hover:to-[#63B3ED]'}`}
                >

...
const Layout = ({ navigation, userRole}) => {
 ...
 // 1. Add a new ref for the User Drawer
 ...
 // 2. Add a new useEffect to close the User Drawer when clicking outside
 useEffect(() => {
  const handleClickOutsideDrawer = (event) => {
   // Check if the drawer is open AND the click is outside the drawer element
   // AND the click is NOT on the profile menu trigger (which opens the drawer)
   ...
   if (showUserDrawer && userDrawerRef.current && !userDrawerRef.current.contains(event.target) && !isClickOnProfileTrigger) {
    ...
    setShowEditForm(false); // Optionally close edit form when closing drawer
   }
  };
 ...
 }, [showUserDrawer]); // Dependency on showUserDrawer
 ...
 return (
  <div className="dashboard-container" data-theme={theme}>
   ...
   {/* User Drawer - 3. Attach the userDrawerRef here */}
   <div 
